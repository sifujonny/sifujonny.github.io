...
document.addEventListener('DOMContentLoaded', function() {
    ...
    displayButton.addEventListener('click', function() {
        ...
        restaurants.forEach(function(restaurant) {
            restaurantList.innerHTML += `<div class="restaurant-card">
                <h3>${restaurant.name}</h3>
                <p>Cuisine: ${restaurant.cuisine} | Rating: ${restaurant.rating} | Price: ${restaurant.priceRange}</p>
                <p>Neighborhood: ${restaurant.neighborhood} | Hours: ${restaurant.hours}</p>
                </div>`;    
    });
});
    ...
    filterButton.addEventListener('click', function() {
        ...
        cheapRestaurants.forEach(function(restaurant) {
            filteredList.innerHTML += `<div class="restaurant-card">
            <h3>${restaurant.name}</h3>
            <p>${restaurant.cuisine} | ${restaurant.priceRange} | Rating: ${restaurant.rating}</p>
            </div>`;
    });    
    });
    ...
    mapButton.addEventListener('click', function() {
        ...
        mappedList.innerHTML = names.map(function(name) {
            return `<p>${name}</p>`;
        }).join('');
    ...
    errorButton.addEventListener('click', async function() {
        ...
        try {
            ...
            <p class="status-message">Error: Could not load the file.</p>
            <p>This happens when a URL doesn't exist or the server is down. In real apps, always use try/catch to handle this gracefully.</p>
            </div>`;
